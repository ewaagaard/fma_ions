...
plot_only_first_and_last_turn = True

# Max zeta for particles
max_zeta = 0.74269329

# Track with 10% BB, space charge and IBS
sps = fma_ions.SPS_Flat_Bottom_Tracker(num_part=10_000, num_turns=2000, turn_print_interval=10)
...
for distribution_type in distribution_types:
    ...
    if plot_only_first_and_last_turn: 
        for i in [0, sps.num_turns-1]:
            ...
            plt.tight_layout()
            fig.savefig('output_plots_f_{}/SPS_Pb_longitudinal_turn_{}.png'.format(distribution_type, i+1), dpi=250)
    else:
