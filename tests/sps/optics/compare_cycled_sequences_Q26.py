"""
Check optics functions of normal Q26 sequence, then cycling the same sequence to point with low D'x --> then compare emittance growth rates
"""
import fma_ions
...
print('Q26 (normal): Qx = {:.3f}, Qy = {:.3f}'.format(twiss['qx'], twiss['qy']))
print('Q26 start values: betx = {:3f} m, bety = {:3f} m, Dx = {:3f} m, Dxprime = {:.3f}m'.format(twiss.betx[0], twiss.bety[0], twiss.dx[0], twiss.dpx[0]))
print('Q26 max values: betx = {:3f} m, bety = {:3f} m, Dx = {:3f} m\n'.format(max(twiss.betx), max(twiss.bety), max(twiss.dx)))

# Q26 optics - cycled
line2 = line.cycle(index_first_element=np.argmin(np.abs(twiss.dpx)))
twiss2 = line2.twiss()

print('Q26 (cycled): Qx = {:.3f}, Qy = {:.3f}'.format(twiss2['qx'], twiss2['qy']))
print('Q26 cycled start values: betx = {:3f} m, bety = {:3f} m, Dx = {:3f} m, Dxprime = {:.3f}m'.format(twiss2.betx[0], twiss2.bety[0], twiss2.dx[0], twiss2.dpx[0]))
print('Q26 cycled max values: betx = {:3f} m, bety = {:3f} m, Dx = {:3f} m\n'.format(max(twiss2.betx), max(twiss2.bety), max(twiss2.dx)))
...
fig1.suptitle(
    r'Q26 uncycled: $q_x$ = ' f'{twiss.qx:.4f}' r' $q_y$ = ' f'{twiss.qy:.4f}' '\n'
...
fig2.suptitle(
    r'Q26 cycled: $q_x$ = ' f'{twiss2.qx:.4f}' r' $q_y$ = ' f'{twiss2.qy:.4f}' '\n'
