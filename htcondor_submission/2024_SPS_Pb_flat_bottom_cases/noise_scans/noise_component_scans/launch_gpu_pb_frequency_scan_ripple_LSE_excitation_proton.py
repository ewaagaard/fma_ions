...
master_name = 'Q26_protons_frequency_ripple_scan_adaptive_SC_excited_LSE_effective_aperture'
num_turns = 60_000
Qx = 26.36
Qy = 26.19
...
tbt = sps.track_SPS(ion_type=='proton', which_context='gpu', distribution_type='qgaussian', install_SC_on_line=True, add_beta_beat=True,
...
sub.copy_plot_script_emittances_for_scan(master_name, folder_names, 
                                         scan_array_for_x_axis='np.hstack((np.arange(10., 100., 10), np.arange(100., 600., 50), np.arange(600., 1201., 100))).ravel()',
                                             ...
                                             extra_text_string='$Q_{x, y}$ = 26.36, 26.19 - q-Gaussian proton beam\\nAdaptive SC, ~10% $\\beta$-beat + non-linear magnet errors\\nExcited LSE')