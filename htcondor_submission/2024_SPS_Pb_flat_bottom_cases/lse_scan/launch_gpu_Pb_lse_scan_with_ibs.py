...
master_name = 'launch_gpu_Pb_lse_scan_with_ibs_and_adaptive_sc'
LSE_strengths = np.array([0.0, 1.0, 3.0, 5.0, 8.0])
num_turns = 130_000 # corresponds to 3s for SPS ions at flat bottom
Qx = 26.31
Qy = 26.19
run_files = ['sps_run_lse_{}_tbt.py'.format(i+1) for i in range(len(LSE_strengths))]
...
tbt = sps.track_SPS(which_context='gpu', distribution_type='qgaussian', install_SC_on_line=True, add_beta_beat=True, add_non_linear_magnet_errors=True, 
                    I_LSE={}, apply_kinetic_IBS_kicks=True, ibs_step = 2000, SC_adaptive_interval_during_tracking=20)
