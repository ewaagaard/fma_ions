...
master_name = 'Q26_Pb_ions_Qx_scan_LSE_exc_0_with_50_150_300_600_ripple_WITH_COMP_TRANSFER_FUNCTION_ibs_adaptive'
num_turns = 130_000 # corresponds to 3s for SPS ions at flat bottom
Qy = 26.19
Qx_range = np.arange(26.28, 26.42, 0.01)
run_files = ['sps_run_qx_{}_tbt_qy_26dot19_with_comp.py'.format(i+1) for i in range(len(Qx_range))]
...
tbt = sps.track_SPS(which_context='gpu', distribution_type='qgaussian', install_SC_on_line=True, add_beta_beat=True, add_non_linear_magnet_errors=True, 
                    add_tune_ripple=True, apply_kinetic_IBS_kicks=True, ibs_step = 2000, ripple_freqs = ripple_freqs,
