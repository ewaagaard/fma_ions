...
master_name = 'Q26_Pb_ions_bunch_intensity_scan_adaptive_sc_ibs_with_NOT_compensated_50_150_300_600_Hz_ripple_130k_turns_TRANSFER_FUNCTION_UNFINISHED'
num_turns = 130_000 # corresponds to about 6 s for SPS ions at flat bottom
Qy = 26.19
Qx = 26.31

no_LEIR_inj = [2, 3, 4, 5, 6, 8]
ind = [0] # which cases to run
...
run_files = ['sps_run_leir_inj_{}.py'.format(i+1) for i in ind]

# Define script and folder names
script_names = run_files.copy()
folder_names = ['sps_no_leir_inj_{}'.format(no_LEIR_inj[i]) for i in ind]
string_array = ['No. LEIR inj. = {}'.format(no_LEIR_inj[i]) for i in ind]    
...
tbt.to_json(output_dir)
    '''.format(num_turns, Nb_array[ind[i]], exn_array[ind[i]], eyn_array[ind[i]], Qx, Qy)
    )
    run_file.close()
