...
master_name = 'Q26_Pb_ions_bunch_intensity_scan_adaptive_sc_130k_turns_no_aperture'
...
run_files = ['sps_run_leir_inj_{}_no_aperture_no_ripple_no_ibs.py'.format(i+1) for i in range(len(no_LEIR_inj))]

# Define script and folder names
script_names = run_files.copy()
folder_names = ['sps_no_leir_inj_{}_no_aperture_no_ripple_no_ibs'.format(no_LEIR_inj[i]) for i in range(len(no_LEIR_inj))]
...
tbt = sps.track_SPS(which_context='gpu', distribution_type='qgaussian', beamParams=beamParams, install_SC_on_line=True, add_aperture=False, use_effective_aperture=False,
                    add_beta_beat=True, add_non_linear_magnet_errors=True, store_particles=True, SC_adaptive_interval_during_tracking=100)
