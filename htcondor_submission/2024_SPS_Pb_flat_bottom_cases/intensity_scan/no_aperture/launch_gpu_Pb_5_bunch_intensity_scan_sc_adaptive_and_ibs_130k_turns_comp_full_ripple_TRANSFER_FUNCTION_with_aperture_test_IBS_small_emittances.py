...
master_name = 'Q26_Pb_ions_bunch_intensity_scan_adaptive_sc_ibs_with_compensated_50_150_300_600_Hz_ripple_130k_turns_TRANSFER_FUNCTION_with_aperture'
num_turns = 130_000 # corresponds to about 6 s for SPS ions at flat bottom
Qy = 26.19
Qx = 26.31

no_LEIR_inj = [2, 4, 8]
Nb_array = ['1.3e8', '2.215e8', '3.94e8']
exn_array = ['0.73e-6', '0.73e-6', '0.73e-6']
eyn_array = ['0.48e-6', '0.48e-6', '0.48e-6']
run_files = ['sps_run_leir_inj_{}_no_aperture_with_ripple_ibs.py'.format(i+1) for i in range(len(no_LEIR_inj))]

# Define script and folder names
script_names = run_files.copy()
folder_names = ['sps_no_leir_inj_{}_no_aperture_with_ripple_ibs'.format(no_LEIR_inj[i]) for i in range(len(no_LEIR_inj))]
...
tbt = sps.track_SPS(which_context='gpu', distribution_type='qgaussian', beamParams=beamParams, install_SC_on_line=True, add_aperture=True, use_effective_aperture=True,
                    add_beta_beat=True, add_non_linear_magnet_errors=True, apply_kinetic_IBS_kicks=True, ibs_step = 2000, store_particles=True,
