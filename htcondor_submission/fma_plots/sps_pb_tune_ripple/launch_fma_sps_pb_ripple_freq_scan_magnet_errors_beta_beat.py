...
master_name = 'Q26_Pb_FMA_on_momentum_ideal_lattice_ripple_scan_magnet_errors_beta_beat'
ripple_freq = [0., 10., 50., 150., 300., 600., 1200.]
Qx = 26.31
Qy = 26.25

run_files, folder_names, string_array = [], [] , []
for i in range(len(ripple_freq)):
    run_files.append('sps_run{}_ripple_magnet_errors_beta_beat.py'.format(i+1))
    folder_names.append('sps_Qx_{:.2f}_Qy_{:.2f}_ripple_{:.3e}_Hz_ripple_magnet_errors_beta_beat'.format(Qx, Qy, ripple_freq[i]))
...
tbt = fma_sps.run_SPS(qx0={:.3f}, qy0={:.3f}, which_context = 'gpu', add_non_linear_magnet_errors=True, add_tune_ripple=True, ripple_freqs=np.array([{}]), add_beta_beat=True)
